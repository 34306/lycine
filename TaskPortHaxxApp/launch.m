...
pid_t launchTest(NSString *excPortName, NSString *arg1, BOOL suspended) {
    ...
    NSMutableDictionary *envVars = [NSMutableDictionary dictionaryWithDictionary:@{
        @"TMPDIR": @"/var/tmp",
        @"HOME": @"/var/root",
    ...
    // Only add exception port name if provided
    if (excPortName) {
        envVars[@"HAXX_EXCEPTION_PORT_NAME"] = excPortName;
    }
